package br.com.vitral.modelo;

import java.io.Serializable;
import java.util.Date;

public class DiaAgendaPortaoModel implements Serializable {

	private static final long serialVersionUID = 1L;

	private Integer id;

	private Date data;

	private FuncionarioModel abertura;

	private FuncionarioModel fechamento;

	private boolean feriado;

	public Integer getId() {
		return id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public Date getData() {
		return data;
	}

	public void setData(Date data) {
		this.data = data;
	}

	public FuncionarioModel getAbertura() {
		return abertura;
	}

	public void setAbertura(FuncionarioModel abertura) {
		this.abertura = abertura;
	}

	public FuncionarioModel getFechamento() {
		return fechamento;
	}

	public void setFechamento(FuncionarioModel fechamento) {
		this.fechamento = fechamento;
	}

	public boolean isFeriado() {
		return feriado;
	}

	public void setFeriado(boolean feriado) {
		this.feriado = feriado;
	}

	@Override
	public String toString() {
		return "DiaAgendaPortaoModel [id=" + id + ", data=" + data + ", abertura=" + abertura + ", fechamento="
				+ fechamento + ", feriado=" + feriado + "]";
	}

	@Override
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((data == null) ? 0 : data.hashCode());
		result = prime * result + ((id == null) ? 0 : id.hashCode());
		return result;
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		DiaAgendaPortaoModel other = (DiaAgendaPortaoModel) obj;
		if (data == null) {
			if (other.data != null)
				return false;
		} else if (!data.equals(other.data))
			return false;
		if (id == null) {
			if (other.id != null)
				return false;
		} else if (!id.equals(other.id))
			return false;
		return true;
	}
	
	
}